...
class dbHandle( ):
    ...
    def _insertGetID( self, cur, tablename, fieldname, value ):
        ...
        if ft is None:
            ...
            return \
            cur.execute( 'SELECT id FROM ' + tablename + ' WHERE ' + fieldname + ' = (?)', (value,) ).fetchone( )[ 0 ]
        else:
            return ft[ 0 ]
    ...
    def insert_csv2DB( self, filename ):
        ...
        for row in csv.reader( f ):
            ...
            # DailyTrades
            self.cur_db.execute( \
                'INSERT OR IGNORE INTO DailyTrades (brokerage_id, stock_id, date_id, price, buy_volume, sell_volume ) VALUES ( ?, ?, ?, ?, ?, ? )', \
