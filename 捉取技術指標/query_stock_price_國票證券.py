...
class Technical_Indicator:
    ...
    def GetKD( self, period = 9, k = 3, d = 3 ):
        ...
        # 计算K值
        self.df[ str_k ] = self.df[ 'rsv' ].ewm( ignore_na = False, adjust = True, com = k, min_periods = 0 ).mean( )

        # 计算D值,
        self.df[ str_d ] = self.df[ str_k ].ewm( ignore_na = False, adjust = True, com = d, min_periods = 0 ).mean( )
